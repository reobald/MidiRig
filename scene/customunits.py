...
#Converts a cc11 expression message on a certain midi channel to an new controller
...
def SetGlobalTranspose():
	return CtrlFilter(79) % ((	(CtrlValueFilter( 64) % Process(transposeReset)) >> \
					...
					(CtrlValueFilter(127) % Process(transposeUp   )) 
				) >> SendOSC(56419, "/system/preview/text", getTransposeMsg))
